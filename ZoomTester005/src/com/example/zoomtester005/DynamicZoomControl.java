...
public class DynamicZoomControl implements Observer {
	...
	public void pan(float dx, float dy) {
		...
		if (mState.getPanX() > panMaxX && dx > 0 || mState.getPanX() < panMinX
				&& dx < 0) {
			dx *= PAN_OUTSIDE_SNAP_FACTOR;
		}
		if (mState.getPanY() > panMaxY && dy > 0 || mState.getPanY() < panMinY
				&& dy < 0) {
	...
	private final Runnable updateRunnable = new Runnable() {
		public void run() {
			...
			final boolean isAtRest = panDynamicsX.isAtRest(
					REST_VELOCITY_TOLERANCE, REST_POSITION_TOLERANCE)
					&& panDynamicsY.isAtRest(REST_VELOCITY_TOLERANCE,
	...
	public void startFling(float vx, float vy) {
		...
		panDynamicsX.setState(mState.getPanX(),
				vx / mState.getZoomX(aspectQuotient), now);
		panDynamicsY.setState(mState.getPanY(),
